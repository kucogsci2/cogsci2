...
def train_or_eval_model(model,loss_function, dataloader, epoch, optimizer=None, train=False):
    ...
    avg_fscore = round(
        f1_score(labels, preds, sample_weight=masks,
            average='weighted') * 100,2)
    
    return avg_loss, avg_accuracy, labels, \
        preds, masks,avg_fscore, [alphas_f, alphas_b, vids]
...
if __name__ == '__main__':
    ...
    # loss_weights = torch.FloatTensor([1/0.6903, 1/0.3097])
    ...
    train_loader, valid_loader, test_loader = get_MOSEI_loaders(
        './data/categorical.pkl', valid=0.0,
    ...
    for e in tqdm(range(n_epochs), desc = 'MOSEI Categorical'):
        train_loss, train_acc, _, _, _, train_fscore, _ = \
            train_or_eval_model(
                model, loss_function, train_loader, e, optimizer, True)

        test_loss, test_acc, test_label, \
            test_pred, test_mask, test_fscore, attentions = \

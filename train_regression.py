...
def get_MOSEI_loaders(path, batch_size=128, valid=0.1, num_workers=0, pin_memory=False):
    ...
    valid_loader = DataLoader(trainset, batch_size=batch_size,
        sampler=valid_sampler, collate_fn=trainset.collate_fn,
...
def train_or_eval_model(model, loss_function, dataloader, epoch, optimizer=None, train=False):    
    ...
    pred_lab = pd.DataFrame(
        list(filter(lambda x: x[2]==1, zip(labels, preds, masks))))
    pear = round(pearsonr(pred_lab[0], pred_lab[1])[0], 4)
    return avg_loss, mae, pear, labels, preds, masks

if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        "Trains a regression model for sentiment data " +
        "with output ranging from -3 to +3 indicating sentiment")
    parser.add_argument(
    ...
    D_m_text, D_m_audio, D_m_video, D_m_context = 300, 74, 35, 300

...
def train_or_eval_model(model, loss_function, dataloader,
    epoch, optimizer=None, train=False):
    ...
    for data in dataloader:
        ...
        # print(textf, visuf, acouf, qmask, umask, label)
        # raise NotImplementedError
        log_prob ,alpha_f,alpha_b  = model(
            textf, acouf, visuf, textf, qmask, umask)
...
if __name__ == '__main__':
    ...
    model = CategoricalModel(
        D_m_text, D_m_audio, D_m_video, D_m_context, D_g, D_p, D_e, D_h,
        n_classes=n_classes, dropout_rec=args.rec_dropout,
        dropout=args.dropout)

    # print(D_m_text, D_m_audio, D_m_video, D_m_context, D_g, D_p, D_e, D_h)
    # raise NotImplementedError
    ...
    train_loader, valid_loader, test_loader = get_MOSEI_loaders(
        './data/' + str(emotion) + '.pkl', valid=0.0,
    ...
    for e in tqdm(
        range(n_epochs), desc = 'MOSEI Emotion (' + str(emotion) + ')'):
    ...
    print('Loss {} accuracy {}'.format(best_loss, round(
        accuracy_score(best_label,best_pred,sample_weight=best_mask)*100,2)))
    print(classification_report(
        best_label, best_pred, sample_weight=best_mask, digits=4))
